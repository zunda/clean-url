...
describe('URL', () => {
  ...
  test('accepts empty input', () => {
    for (const x of [undefined, null, ""]) {
      expect(URL.parse(x)).toBe(null)
    }
  })

  test('accepts non-URL input', () => {
    expect(URL.parse("Hello, World!")).toBe(null)
  })

  test('parses a full URL', () => {
    const parsed = URL.parse('https://user:pass@www.example.com/path/to?p=parameter&q=query&p=2ndparameter#fragment')
    const expected = {
      hash: '#fragment',
  ...
  test('parses an http URL with non-standard port', () => {
    const parsed = URL.parse('http://example.com:8080')
  ...
  test('parses an https URL with non-standard port', () => {
    const parsed = URL.parse('https://example.com:80')
  ...
  test('doesn\'t parse a URL queries spearated by smicolon', () => {
    const parsed = URL.parse('https://example.com/?p=parameter;q=query')
    const pars = [
      ['p', 'parameter;q=query'],
    ]
    var i = 0
